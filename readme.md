# Work
## Secret Language App
```
import java.util.Scanner;

public class Work {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("Welcome to word encode-decode");
        System.out.println("1. Encode a string");
        System.out.println("2. Decode a string");
        System.out.println("Chosose and option(1 or 2)");
        int choice = sc.nextInt();
        sc.nextLine();
        System.out.println("Enter a string: ");
        String input = sc.nextLine();
        if(choice==1){
            String encodedString=encode(input,0);
            System.out.println("Encode string is : " + encodedString);
        }else{
            String decodedString=decode(input,0);
            System.out.println("Decode string is : " + decodedString);
        }
        
    }
    
    public static String encode(String input,int index){
        if(index==input.length()){
            return "";

        }
        char ch = input.charAt(index);
        char encodeChar = atbush(ch);
        return encodeChar+encode(input,index+1);
    
    }
    public static String decode(String input,int index){
        if(index==input.length()){
            return "";
        }
        char ch =input.charAt(index);
        char decodeChar = atbush(ch);
        return decodeChar+decode(input, index+1);
    }
    public static char atbush(char ch){
        if(ch>='a'&& ch<='z'){
            return(char)('z'-(ch-'a'));

        }else if (ch>='A' && ch<='Z'){
            return(char)('Z'-(ch-'A'));
        }else{
            return ch;
        }
    }
    }
```
[githublink](https://github.com/Aromalpriyan/Work)